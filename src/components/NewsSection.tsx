...
const NewsSection = () => {
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 sm:mb-12">
      ...
      <button className="text-sm sm:text-base text-gray-400 hover:text-white transition-colors mt-2 sm:mt-0">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-16">
      {newsItems.map((item, index) => (
        <div key={index} className="group cursor-pointer">
          ...
          <h3 className="text-base sm:text-lg font-semibold mb-1 sm:mb-2 group-hover:text-yellow-400 transition-colors">
            {item.title}
          </h3>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12">
      ...
      <div>
        <h3 className="text-xl sm:text-3xl font-bold mb-6 sm:mb-8 text-white">
          Upcoming & recent events
        </h3>
          ...
          {events.map((event, index) => (
            <div key={index} className="flex items-start space-x-3 sm:space-x-4 group cursor-pointer">
              ...
              <div className="flex-1">
                <h4 className="text-sm sm:text-base font-semibold group-hover:text-yellow-400 transition-colors mb-1">
                  {event.title}
                </h4>
      ...
      <div>
        <h3 className="text-xl sm:text-2xl font-bold mb-0 sm:mb-16 text-white"></h3>
        <div className="space-y-4 sm:space-y-6">
          {events.map((event, index) => (
            <div key={index} className="flex items-start space-x-3 sm:space-x-4 group cursor-pointer">
              <div className="w-4 h-4 sm:w-6 sm:h-6 bg-yellow-400 rounded flex-shrink-0 mt-1"></div>
